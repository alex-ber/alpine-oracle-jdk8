#fork from https://github.com/anapsix/docker-alpine-java/blob/master/8/111b14/jdk/unlimited/Dockerfile
...
#Alex
#see https://mail-tp.fareoffice.com/java/
#COPY ${JAVA_PACKAGE}-${JAVA_VERSION_MAJOR}u${JAVA_VERSION_MINOR}-linux-x64.tar.gz /tmp/java.tar.gz
     curl -sSL https://mail-tp.fareoffice.com/java/${JAVA_PACKAGE}-${JAVA_VERSION_MAJOR}u${JAVA_VERSION_MINOR}-linux-x64.tar.gz -o /tmp/java.tar.gz && \
...
#    sha256sum -c /tmp/java.tar.gz.sha256 && \
    ...
    #--Alex Git added
    apk --no-cache add git=2.18.1-r0 less=530-r0 openssh=7.7_p1-r4 && \
    #--Alex apk del curl glibc-i18n && \
    apk del glibc-i18n && \
...
#docker build --squash . -t first_base_alpine_oracle_jdk_8
#docker run -d first_base_alpine_oracle_jdk_8 bash
#based on https://blog.developer.atlassian.com/minimal-java-docker-containers/
#docker export $(docker ps -q -n=1) | docker import - prebase_alpine_oracle_jdk_8
#docker run -d prebase_alpine_oracle_jdk_8 bash
#based on https://docs.docker.com/engine/reference/commandline/commit/
#populate from docker inspect -f "{{ .Config.Env }}" first_base_alpine_oracle_jdk_8
# docker commit --change "ENV PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin:/opt/jdk/bin \
...
#    JAVA_JCE=unlimited \
#    JAVA_HOME=/opt/jdk" \
#    $(docker ps -q -n=1) alexberkovich/alpine-oralce-jdk8

#smoke test
#docker run -it alexberkovich/alpine-oralce-jdk8 java -version
#docker tag alexberkovich/alpine-oralce-jdk8 alexberkovich/alpine-oralce-jdk8:0.0.1
#docker push alexberkovich/alpine-oralce-jdk8
# EOF
